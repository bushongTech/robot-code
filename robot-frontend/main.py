...
- This is the human-facing UI.
- Sends commands to ROBOT_CMD_BC (broadcast so any controller can listen, but we’ll have exactly one).
- Subscribes to ROBOT_TLM to watch pose/process.
- No RoboDK here — keep the UI light and portable.

Flow at a glance:
1) Read YAML (broker/exchanges/queues).
2) Connect to LavinMQ, make sure exchanges/queues exist.
3) Start background consumer on ROBOT_TLM queue so UI stays snappy.
4) Tk window: freeform JSON, jog buttons, STOP/PAUSE, GoTo form.
"""
...
# --- AMQP wrapper (UI-side: declare, publish JSON, consume TLM) ---
class AMQPClient:
    ...
    # open connection/channel once; reuse for everything
    def connect(self):
    ...
    # idempotent infra: exchanges/queues from YAML so we don’t drift
    ...
    # background consume loop: feed handler, ack after it returns
    ...
    # clean shutdown so the consume loop stops without stack traces
    def close(self):
        self._closing = True
        try:
...
# quick helpers so main() reads clean
def load_config():
    """Read the YAML; we want broker/exchanges/queues driven by file, not code."""
    with open(CONFIG_PATH, "r", encoding="utf-8") as f:
        return yaml.safe_load(f)

def first_queue_for_exchange(cfg, exchange_name):
    """Find the first queue bound to an exchange. Good enough for our UI."""
    for ex in cfg["brokers"]["lavinmq"].get("exchanges", []):
        if ex.get("name") == exchange_name:
            qs = ex.get("queues", [])
            return qs[0]["name"] if qs else None
    return None


# All UI in one class: freeform JSON, jog, STOP/PAUSE, GoTo, status, and a FIFO drain
class RobotUI:
    def __init__(self, amqp, cmd_exchange, tlm_fifo):
        ...
        self.root = tk.Tk()
        self.root.title("Robot Command Console")
        ...
        # Freeform JSON sender (handy for oddball commands)
        ...
        # Drain telemetry FIFO periodically so it never grows unbounded
        self._pump_fifo()

    # tiny publisher helper so button handlers stay small
    ...
    # stop/pause are simple one-word commands
    def stop(self):
    ...
    # absolute move with speed/units
    def go_to(self):
        payload = {
            "cmd": "goto",
    ...
    # keep the UI responsive and memory stable
    def _pump_fifo(self):
        while not self.tlm_fifo.empty():
...
def main():
    """Wire-up: read config -> connect -> declare -> start TLM consumer -> start UI."""
    cfg = load_config()
    amqp = AMQPClient(cfg)
    amqp.connect()
    amqp.declare_from_config()

    tlm_queue = first_queue_for_exchange(cfg, "ROBOT_TLM") or "robot-telemetry"

    tlm_fifo = queue.Queue()

    # shove telemetry into a FIFO and also print for visibility
    def on_tlm(msg):
        tlm_fifo.put(msg)
